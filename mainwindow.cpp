...
void MainWindow::on_inicio_clicked()
{
    //int id = ui->id->value();
    bool ok;
    QString sid = ui->line_ID->text();
    int id = sid.toInt(&ok);
    QString svolt = ui->line_V->text();
    float volt = svolt.toFloat(&ok);
    QString sx = ui->line_X->text();
    int posx = sx.toInt(&ok);
    QString sy = ui->line_Y->text();
    int posy = sy.toInt(&ok);
    QString sr = ui->line_Y->text();
    int red = sr.toInt(&ok);
    QString sb = ui->line_Y->text();
    int blue = sb.toInt(&ok);
    QString sg = ui->line_Y->text();
    int green = sg.toInt(&ok);
    string r = "Id repetido, por favor ingrese otro";
    string s = "Neurona ingresada";
    string c = "Uno de los colores tiene un valor mayor a 255 por favor introduzca un valor menor o igual...";
    if(admin.validar_id(id)){
        ui->pantallaid->setPlainText(QString::fromStdString (r));
    }
    else{
        if(red>255 or blue>255 or green>255){
            ui->pantallaid->setPlainText(QString::fromStdString (c));
        }
        else{
...
void MainWindow::on_dibujar_button_clicked()
{
    ...
    for(int i=0;i<admin.aux;i++){
        ...
        escena.addEllipse(admin.vect[i].posicion_x,admin.vect[i].posicion_y,3,3,pen);
...
void MainWindow::on_conexion_neuronas_clicked()
{
    QPen pen;
    pen.setWidth(1);
    float d=5000;
    ...
    for(int i=0;i<admin.aux;i++){
        d=5000;
        int x1 = admin.vect[i].posicion_x;
        int y1 = admin.vect[i].posicion_y;
        for(int j=0;j<admin.aux;j++){
            if(j==i){
               //no hace nada
            }
            else{
                int x2 = admin.vect[j].posicion_x;
                int y2 = admin.vect[j].posicion_y;
                double dx = x1 - x2;
                double dy = y1 - y2;
        ...
        escena.addLine(xf1,yf1,xf2,yf2,pen);
        ui->PantallaNeuronas->setScene(&escena);
    }
}
